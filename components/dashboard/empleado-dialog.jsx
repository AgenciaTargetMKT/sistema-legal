...
export default function EmpleadoDialog({
    ...
    const crearUsuarioAuth = async (email, telefono, nombre, apellido) => {
        try {
            ...
            console.log('Creando usuario Auth con:', { email, password: '***' });
    ...
    const eliminarUsuarioAuth = async (authUserId) => {
        try {
            ...
            console.log('Eliminando usuario Auth:', authUserId);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (empleado) {
                ...
                const cambioAInactivo = empleado.activo === true && formData.activo === false;
                const cambioAActivo = empleado.activo === false && formData.activo === true;

                // Si se desactiva el empleado Y tiene auth_user_id
                if (cambioAInactivo && empleado.auth_user_id) {
                    try {
                        console.log('Desactivando empleado, eliminando usuario Auth...');
                        ...
                        if (!confirm("No se pudo eliminar el usuario de autenticación. ¿Desea continuar con la desactivación?")) {
                            setLoading(false);
                            return;
                        }
                        updateData.auth_user_id = null;
                ...
                // Si se activa el empleado Y NO tiene auth_user_id
                    ...
                    try {
                        console.log('Activando empleado, creando usuario Auth...');
                        ...
                        console.log('Usuario Auth creado correctamente:', nuevoAuthUserId);
                    } catch (authError) {
                        console.warn("No se pudo crear usuario Auth:", authError);
                        if (!confirm("No se pudo crear el usuario de autenticación. ¿Desea continuar con la activación sin acceso al sistema?")) {
                            setLoading(false);
                            return;
                        }
                    }
                ...
                if (formData.activo) {
                    try {
                        console.log('Creando nuevo empleado activo con usuario Auth...');
                        ...
                        console.warn("No se pudo crear usuario auth:", authError);
                        if (!confirm("No se pudo crear el usuario de autenticación. ¿Desea continuar creando el empleado sin acceso al sistema?")) {
                            setLoading(false);
                            return;
                        }
                    }
                } else {
                    console.log('Creando nuevo empleado inactivo sin usuario Auth');
                }
                ...
                if (formData.activo && authUserId) {
                    alert("Empleado creado exitosamente. La contraseña temporal son los primeros 6 dígitos del teléfono.");
                } else if (formData.activo && !authUserId) {
                    alert("Empleado creado sin acceso al sistema (no se pudo crear usuario de autenticación).");
                } else {
                    alert("Empleado creado como inactivo (sin acceso al sistema).");
                }
            }
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-gray-500">
                                Los primeros 6 dígitos serán la contraseña temporal
                            </p>
                        </div>
