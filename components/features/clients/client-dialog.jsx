...
export default function ClienteDialog({
  ...
  const consultarDNI = async (dni) => {
    ...
    try {
      const response = await fetch(`/api/consulta-dni?numero=${dni}`, {
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Error en la consulta: ${response.status}`
        );
      }
      ...
      if (data.first_name && data.first_last_name) {
        const nombreCompleto = `${data.first_name} ${data.first_last_name}${
          data.second_last_name ? ` ${data.second_last_name}` : ""
        }`;
      ...
      toast.error(`Error al consultar DNI: ${error.message}`);
      return null;
    } finally {
  ...
  const consultarRUC = async (ruc) => {
    ...
    try {
      const response = await fetch(`/api/consulta-ruc?numero=${ruc}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      toast.error(`Error al consultar RUC: ${error.message}`);
      return null;
    } finally {
  ...
  const handleConsultarManual = async () => {
    if (
      formData.documento_identidad.length === 8 &&
      /^\d+$/.test(formData.documento_identidad)
    ) {
      ...
      if (nombreCompleto) {
        ...
        toast.error("No se pudo encontrar información para el DNI ingresado");
      }
    } else {
  ...
  const handleConsultarRucManual = async () => {
    if (
      formData.documento_identidad.length === 11 &&
      /^\d+$/.test(formData.documento_identidad)
    ) {
      ...
      if (datosEmpresa) {
        ...
        toast.error("No se pudo encontrar información para el RUC ingresado");
      }
    } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              {consultandoRuc && (
                <p className="text-sm text-blue-600">
                  Consultando información del RUC...
                </p>
              )}
              {formData.tipo_cliente === "persona_natural" && (
                <p className="text-xs text-gray-500">
                  Ingrese 8 dígitos y presione "Consultar" para buscar
                  automáticamente
                </p>
              )}
              {formData.tipo_cliente === "empresa" && (
                <p className="text-xs text-gray-500">
                  Ingrese 11 dígitos y presione "Consultar" para buscar
                  automáticamente
                </p>
              )}
            </div>
