...
export async function GET(request) {
  ...
  if (!numero || numero.length !== 11 || !/^\d+$/.test(numero)) {
    return new Response(
      JSON.stringify({ error: 'RUC inválido' }),
  ...
  try {
    ...
    const response = await fetch(
      `https://api.decolecta.com/v1/sunat/ruc?numero=${numero}&token=${apiKey}`,
      {
        method: "GET",
        headers: {
    ...
    return new Response(
      JSON.stringify({ error: 'Error al consultar el RUC' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
}
